...
: unexp ( pin --  )
    ...
    if
      ...
      fail$ type ." unexport for writing!"
    then
    nip
    close
;
...
: dir ( pin direction --  )
    ...
    rot  ( direction 1 pin )
    <# 0 hold dir$ #$ #s gpio$ #$
    root$ #$ #> drop open ( direction fs )
    dup 0> ( direction fs flag)
    if
      dup ( direction fs fs )
      rot ( fs fs direction )
      ...
      fail$ type ." direction for writing!"
    then
    nip
    close
;
